            ...
            google =
              {
                Gpx_plot.Model.x =
                  float_of_string
                    (List.nth (List.nth the_widgets.google_coords 0) i)#text;
                y =
                  float_of_string
                    (List.nth (List.nth the_widgets.google_coords 1) i)#text;
              };
            co =
              {
                Gpx_plot.Model.x =
                  float_of_string
                    (List.nth (List.nth the_widgets.co_coords 0) i)#text;
                y =
                  float_of_string
                    (List.nth (List.nth the_widgets.co_coords 1) i)#text;
              };
          })
        range
    in
    ...
    let () =
      List.iter
        (fun i ->
          ...
          in
          let () =
            (List.nth (List.nth w.google_coords 0) i)#set_text
              (Printf.sprintf "%f" value.Gpx_plot.Model.x)
          in
          let () =
            (List.nth (List.nth w.google_coords 1) i)#set_text
              (Printf.sprintf "%f" value.Gpx_plot.Model.y)
          in
          ())
        range
    in
          ...
          let value =
            if i < List.length project.Gpx_plot.Model.common_points then
              let common_point =
                List.nth project.Gpx_plot.Model.common_points i
              in
              common_point.Gpx_plot.Model.co
            else { Gpx_plot.Model.x = 0.0; y = 0.0 }
          in
          let () =
            (List.nth (List.nth w.co_coords 0) i)#set_text
              (Printf.sprintf "%f" value.Gpx_plot.Model.x)
          in
          let () =
            (List.nth (List.nth w.co_coords 1) i)#set_text
              (Printf.sprintf "%f" value.Gpx_plot.Model.y)
          in
          ())
        range
...
let main () =
  ...
  try
    ...
    (* co maps points *)
    let entries_co : GEdit.entry list list =
      List.map
        (fun (row, t) ->
          ...
          let entries =
            List.map
              (fun i ->
                ...
                let _ = table#attach ~left:(i + 2) ~top:row entry#coerce in
                let _ =
                  entry#connect#activate ~callback:(comap_entry (i + 1) t entry)
                in
                entry)
              range
          in
